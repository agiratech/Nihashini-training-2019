import { Component, OnInit, ViewChild, ElementRef, Input, Output, EventEmitter, OnChanges, SimpleChanges, OnDestroy} from '@angular/core';
...
import { AudienceTitleDialogComponent } from '@shared/components/audience-title-dialog/audience-title-dialog.component.ts';
import {debounceTime, distinctUntilChanged, map, startWith, tap, takeUntil} from 'rxjs/operators';
@Component({
  selector: 'app-audience-browser',
  templateUrl: './audience-browser.component.html',
  styleUrls: ['./audience-browser.component.less'],
})
export class AudienceBrowserComponent implements OnInit, OnChanges, OnDestroy {
  behaviorOption: any = {};
  ...
  public constructor(
    ...
    private auth: AuthenticationService, private commonService: CommonService, private filtersService: FiltersService,
    private dialog: MatDialog, private theme: ThemeService) {
    this.targetAudienceService
      .getDefaultAudience()
      .subscribe(audience => this.defaultAudience = audience);
      this.searchSavedQuery = new FormControl('');
      this.savedAudienceFilter$ = this.searchSavedQuery
        .valueChanges.pipe(
          debounceTime(300),
          startWith(''));
  }
  public ngOnInit() {
    ...
    this.targetAudienceService.getAudiencesFromApi({}, true)
      .subscribe(result => {
      ...
      // checking the tabs which have to be displayed in select audience
     /* this.audiences = this.audiences.filter((element) => {
        if (this.mod_permission.features.gpAudience.tabs.indexOf(element.label) !== -1) {
          return element;
        }
      });*/
      this.audienceTabUpdate(this.audiences).then( result => {
        setTimeout(() => {
          const defaultAudienceTab = this.filtersService.getExploreSession();
        if ((defaultAudienceTab && defaultAudienceTab['clickAudience'] && defaultAudienceTab['clickAudience'] === true) || localStorage.getItem('clickAudience')=== 'true') {
      ...
      this.selectedAudienceTab = (sessionFilter != null &&
         typeof sessionFilter.audienceTabPosition !== 'undefined') ? sessionFilter.audienceTabPosition : 0;
      ...
      if (this.selectedAudienceList && this.selectedAudienceList.length > 0) {
        ...
        if (typeof currenttab['tabType'] !== 'undefined' && currenttab['tabType'] === 'single') {
          this.onFlatOptionChange(this.selectedAudienceList[0]);
        } else {
          this.targetAudienceService.getAudiencesFromApi({tags: data}, true).subscribe(resultData => {
  ...
  async audienceTabUpdate(audienses) {
    this.audiences = audienses.filter((element) => {
      if (this.mod_permission.features.gpAudience.tabs.indexOf(element.label) !== -1) {
        return element;
      }
    });
  ...
  public clearAll(clearType) {
    ...
    this.targetAudienceService.getAudiencesFromApi().subscribe(result => {
      ...
      if (this.mod_permission
        ...
        && this.mod_permission.features.orderInventories.status
        && this.mod_permission.features.orderInventories.status !== 'active') {
        this.audiences = this.getFormattedAudienceData(result.catalog).filter(audience => audience.label.toLowerCase() !== 'ads');
      } else {
        this.audiences = this.getFormattedAudienceData(result.catalog);
      }
    }, error => {
  ...
  public selectTarget(selectedAudience, event = null) {
    if (this.selectionType === 'multiple') {
      if (event && !event.checked) {
        ...
        const selectedindex = this.selectedTargets.findIndex(audience => audience._id === selectedAudience._id);
  ...
  private setAudienceTitle() {
    if (this.savedAudience && this.savedAudience.length > 0) {
      this.audienceTitleName = this.formatService.getObjectTitle(this.savedAudience, 'Audience', 'title');
    ...
    if (this.selectionType === 'multiple') {
      if (filterType === 'custom' && currentTab['finalTargetAudience']) {
        ...
        this.selectedTargets.map(aud => {
          audiences.push({audience: aud.audiences[0]['key'], name: aud.title});
        });
        this.onCompletedBrowsing.emit({
      ...
      } else if (filterType === 'single' && typeof currentTab['selectedAudience'] !== 'undefined') {
        const audiences = [];
        currentTab['selectedAudience'].map(aud => {
          audiences.push({audience: aud.targetAudienceKey, name: aud.description});
        });
        this.onCompletedBrowsing.emit({
          targetAudience: audiences,
          selectedAudienceList: [],
        ...
        swal('Warning', 'Please further refine your target audience to apply.', 'warning');
      }
    } else {
      if (filterType === 'custom' && currentTab['finalTargetAudience']) {
        this.onCompletedBrowsing.emit({
        ...
        this.onCompletedBrowsing.emit({
          targetAudience: {audience: this.selectedTarget.audiences[0]['key'], name: this.selectedTarget.title},
          selectedAudienceList: [],
          tabPosition: this.selectedAudienceTab,
      ...
      } else if (filterType === 'single' && typeof currentTab['selectedAudience']['tag'] !== 'undefined') {
        const params = {};
        params['tags'] = currentTab['selectedAudience']['tag'];
        params['catalog'] = currentTab['label'];
        this.targetAudienceService.getAudiencesFromApi(params, true).subscribe(result => {
          this.onCompletedBrowsing.emit({
            targetAudience: {audience: result['targetAudienceKey'], name: currentTab['selectedAudience']['description']},
            selectedAudienceList: result['audienceFilter'][0]['options'][0]['options'],
            tabPosition: this.selectedAudienceTab,
            currentTargetKey: result['targetAudienceKey'],
            currentTargetId: currentTab['selectedAudience']['targetAudienceKey'],
            editAudienceId: currentTab['currentAudienceId'],
            clearFilter: false,
            tabType: filterType
          });
  ...
  /*public applySingleAudience() {
    if (this.behaviorOption) {
      const currentTab = this.getCurrentTabData();
      const data = {
        tags: this.behaviorOption.tag,
        catalog: currentTab.label,
      };
      this.targetAudienceService
        .getAudiencesFromApi(data)
        .subscribe(result => {
        });
    } else {
      swal('Please select any audience');
    }
  }*/
  public flatTabSearch(eventText) {
    if (eventText) {
      ...
      if (currentTab.selectedCategory &&
        currentTab.selectedCategory.label !== '') {
        data.catalog = currentTab.label + '.' + currentTab.selectedCategory.label;
      }
      this.targetAudienceService
        .searchAudiences(data)
        .subscribe(result => {
  ...
  public onSearch() {
    ...
    if (searchTerm) {
      ...
      this.targetAudienceService.searchAudiences(data)
        .subscribe(result => {
          this.updateFilteredDataCurrentTab(result.data);
        }, error => {
          swal('Warning', 'Some error occured. Please try again', 'warning');
      ...
      const index = this.selectedAudienceTab - 1;
      ...
      grandParent.options.map(parent => {
        parent.options.map((option) => {
          flattenedSelect.push({
            tag: option.tag, description: option.description, category: grandParent.label, subCategory: parent.label, tab: tabName});
        });
      });
    });
    flattenedAudienceKeys = flattenedSelect.map((option) => option.tag);
    return {'flattenedSelect': flattenedSelect, 'flattenedAudienceKeys': flattenedAudienceKeys};
  }
  ...
  public saveAudience() {
    ...
    if (currentTab['tabType'] === 'single') {
      if (typeof currentTab['selectedAudience']['tag'] === 'undefined') {
         return swal('Warning', 'Please further refine your target audience to apply.', 'warning');
      }
      const params = {};
      params['tags'] = currentTab['selectedAudience']['tag'];
      params['catalog'] = currentTab['label'];
      this.targetAudienceService
        .getAudiencesFromApi(params, true)
        .subscribe(result => {
      ...
      if (!currentTab['finalTargetAudience'] || !currentTab['finalTargetAudience'].audience) {
        return swal('Warning', 'Please further refine your target audience to apply.', 'warning');
      }
      const audienceData: any = {
        audience: {
  ...
  private updateSavedAudience(audienceData: any, currentTab) {
    this.targetAudienceService
      ...
      .subscribe(resultData => {
        ...
        if (typeof error.error !== 'undefined' && error.error['code'] === 3026) {
          swal('Info', 'This saved audience name already exists. Please use a different name.', 'warning');
        } else {
          swal('Warning', 'Some error occured. Please try again', 'warning');
        }
      });
  ...
  private createSaveAudience(audienceData) {
    ...
    this.dialog.open(AudienceTitleDialogComponent, {
      /*height: '160px',*/
      data: audienceData,
      width: '500px',
      ...
      if (typeof result !== 'undefined' && result) {
        ...
        this.targetAudienceService.saveAudience(audienceData).subscribe(audience => {
          if (this.isScenario) {
            this.onCompletedBrowsing.emit({
          ...
          if (typeof error.error !== 'undefined' && typeof error.error['code'] !== 'undefined' &&
  ...
  public editAudience(audience) {
    ...
    if (currentTab['tabType'] === 'single') {
      ...
      this.targetAudienceService.getAudiencesFromApi({catalog: catalog, tags: data}, true).subscribe(res => {
  ...
  public deleteAudience(audience) {
    swal({
      ...
      if (typeof x.value !== 'undefined' && x.value) {
        ...
        this.targetAudienceService.deleteAudience(audience._id).subscribe(response => {
            // this.savedAudience.splice(this.savedAudience.indexOf(audience), 1);
            this.savedAudience$.pipe(takeUntil(this.unSubscribe)).subscribe( savedAudience => {
            ...
            if (Object.keys(this.selectedTarget).length > 0 && this.selectedTarget._id === audience._id) {
              this.clearAll('clear');
            }
          },
          e => {
  ...
  public tabSwitch(event: object = {}) {
    ...
    if (
      ...
      this.targetAudienceService
        .getAudiencesFromApi({catalog: data.label}, true)
        .subscribe(result => {
          if (result['catalog'] && result['catalog'].length > 0) {
            // console.log(result.catalog[0]['options']);
            // sorting based on size not needed
            // result.catalog[0]['options'].forEach(function (element, Index) {
            //   result.catalog[0]['options'][Index].array_length = element.options;
            // });
            result.catalog[0]['options'].sort(this.compareValues('description', 'asc'));
            ...
            if (audience && audience.length >= 1) {
              ...
              data.categories = [];
              let j = 0;
              data.categories.push({ 'description': 'All Categories', 'label': '', 'options': [] });
              audience.map(item => {
              ...
              //   data.categories.push(item);
              //   if (j > 0) {
              //     data.displayList.push(...item.options);
              //   }
              //   j++;
              // });
              // if (data.categories[0] && data.categories[0].label === '') {
              //   data.categories[0]['description'] = 'All Categories';
              // }
  ...
  private loadSavedAudiences(resetTitle = true) {
    this.savedAudience$ = this.targetAudienceService.getFormattedSavedAudiences()
      .pipe(
        tap(savedAudiences => {
          if (this.selectedTarget['_id']) {
            this.selectedTarget = this.targetAudienceService.findSavedAudienceById(this.selectedTarget['_id'], savedAudiences);
          }
          if (resetTitle) {
            this.setAudienceTitle();
          }
      ...
      .pipe(map(([savedAudience, filter]) => {
        return savedAudience.filter(audience => {
          return audience.title.toLowerCase().indexOf(filter.toLowerCase().trim()) >= 0;
        });
  ...
  getFormattedAudienceData(audiences) {
    ...
    for (let i = 0; i < audiences.length; i++) {
      if (typeof audiences[i]['options'][0]['options'] !== 'undefined'
  ...
  public updateDataCurrentTab(r, index = null, savedAudience = {}) {
    index = index != null ? index : this.selectedAudienceTab - 1;
    const data = this.audiences[index];
    data['displayList'] = r.catalog[0] ? r.catalog[0]['options'] : [];
    data['selectedAudience'] = r.audienceFilter[0] ? r.audienceFilter[0]['options'] : [];
    data['targetAudience'] = r.targetAudienceKey;
    const flatten = this.flattenSelection(data['selectedAudience'], data['label']);
    data['flattenedAudienceKeys'] = flatten['flattenedAudienceKeys'];
    data['flattenedSelect'] = flatten['flattenedSelect'];
    const title = flatten['flattenedSelect'].map((option) => option.subCategory + ' - ' + option.description).join(', ');
    ...
    data['finalTargetAudience'] = {
      ...
      name: flatten['flattenedSelect'].map((option) => option.description).join()};
    this.audiences[index] = data;
  }
  onFlatOptionChange(character, index = null) {
    if (index === null) {
      index = this.selectedAudienceTab - 1;
    }
    const data = this.audiences[index];
    ...
    this.targetAudienceService
      ...
      .subscribe(result => {
        data['selectedAudience']['targetAudienceKey'] = result['targetAudienceKey'];
      });
    this.audiences[index] = data;
  }
  onSelectMultipleOption(character, index = null) {
    ...
    const data = JSON.parse(JSON.stringify(this.audiences[index]));
  ...
  checkIsSelected(option, index= null) {
    if (index === null) {
      index = this.selectedAudienceTab - 1;
    }
    const matches = this.audiences[index]['selectedAudience'].filter(v => v.tag === option.tag);
  ...
  private setAudienceFromSession() {
    const filters = (this.isScenario || this.isInventory) ? '' : this.filtersService.getExploreSession();
    if (filters) {
      if (typeof filters['data']  !== 'undefined' && typeof filters['data']['audience'] !== 'undefined') {
        if (typeof filters['data']['audience']['details'] !== 'undefined') {
          const targetDetail = filters['data']['audience']['details'];
          this.exploreData.setSelectedTarget(targetDetail['targetAudience'].audience);
          this.exploreData.setSelectedTargetName(targetDetail['targetAudience'].name);
          ...
          switch (targetDetail['tabType']) {
            case 'single':
              ...
              setAudience.audiences.push(
                {'tag': targetDetail['targetAudience'].audience, 'description': targetDetail['targetAudience'].name});
              this.updateFlatOptionChange(setAudience);
              break;
            case 'custom':
              const data = [];
              const currentTab = this.audiences[targetDetail['tabPosition'] - 1];
              this.selectedAudienceTab = targetDetail['tabPosition'];
              if (currentTab) {
                targetDetail['selectedAudienceList'].forEach(character => {
                  data.push(character.tag);
                });
                this.targetAudienceService.getAudiencesFromApi({catalog: currentTab['label'], tags: data}).subscribe(res => {
              ...
              break;
            case 'saved':
                if (typeof filters['data']['audience']['details']['currentTargetId'] !== 'undefined' &&
                 filters['data']['audience']['details']['currentTargetId'] != null) {
                  this.targetAudienceService.getFormattedSavedAudiences().pipe(takeUntil(this.unSubscribe)).subscribe((audiences) =>{
                    const id = targetDetail['currentTargetId'];
                    if (id) {
                      this.selectedTarget = this.targetAudienceService.findSavedAudienceById(id, audiences);
                    }
                  });
                }
              break;
