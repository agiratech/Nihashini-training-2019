...
export class ExploreDataService {
  ...
  private selectedGeoLocationIdValue = new Subject();
  ...
  public getCSVHeaders(isTabular = false, customizeColumn = []) {
    const CSVHeasers = {
  ...
  public getStaticMapImage(coOrds, width, height) {
    ...
    return 'https://api.mapbox.com/styles/v1/intermx/cj2qj1kf8003k2rmr4dz8mzn0/static/url-' + icon +
      '(' + coOrds[0] + ',' + coOrds[1] + ')/' +
      coOrds[0] + ',' + coOrds[1] + '40.45704,10.0,0,0/' + width + 'x' + height + '?access_token=' + environment.mapbox.access_token;
  }


  // This two methods are for arrow navigation of market filters
  public getMapViewPostionState(): Observable<any> {
    return this.mapViewPostionState.asObservable();
  }

  public setMapViewPostionState(state) {
  ...
  /**
   setter and getter method for selectedPlacesCtrlValue and radiusCtrlValue to handle place based search.
   **/
  public setSelectedMarketLocationValue(value) {
    this.selectedMarketLocationValue.next(value);
  }

  public getSelectedMarketLocationValue() {
    return this.selectedMarketLocationValue.asObservable();
  }

  public setSelectedGeoLocationIdValue(value) {
    this.selectedGeoLocationIdValue.next(value);
  }

  public getSelectedGeoLocationIdValue() {
    return this.selectedGeoLocationIdValue.asObservable();
  }
