...
export class FilterByIdsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.filtersService.checkSessionDataPushed()
      ...
      // setting invalid Ids for geoPanelId and plantUnitIds
      this.filtersService.getFilters()
      .pipe(debounceTime(200),
      distinctUntilChanged())
      .subscribe((filterData) => {
        if (filterData.data.geoPanelId || filterData.data.plantUnitId) {
          this.exploreDataService.getInvalidIds().subscribe((data: any) => {
            if (Object.keys(data).length) {
              this.invalidGeoPanelIds = {type: 'geoPanelId', data: data['geoPanelIds']};
              this.invalidPlantUnitIds = {type: 'plantUnitIds', data: data['plantIds']};
  ...
  removedIdsStatus(status: any) {
    if (status) {
      this.removedInvalidChipsStatus = true;
    }
}
