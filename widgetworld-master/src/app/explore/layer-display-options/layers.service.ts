...
import { FormatService, ThemeService } from '@shared/services';
...
export class LayersService {
   ...
   * @param value
   */
  public setApplyLayers(value: boolean) {
   ...
   * @param layers
   */
  public saveLayersSession(layers, placesModule = false) {
    if (placesModule) {
   ...
   */
  public getlayersSession(placesModule = false) {
    if (placesModule) {
  ...
  // To remove last index color from colors array
    ...
    Object.keys(data['data']).reverse().forEach((key, index) => {
      ...
      value.forEach(market => {
        ...
        colorCode.push(market.id, colors[index]);
        ...
        zipColors[colors[index]].push(market.id);
    ...
    map.on('mouseenter', 'marketPanel', () => {
      map.getCanvas().style.cursor = 'pointer';
    });
    map.on('mouseleave', 'marketPanel', () => {
  ...
  loadTopZipCode(data, map: mapboxgl.Map, color: string, flag: string) {
    ...
    Object.keys(data['data']).reverse().forEach((key, index) => {
      ...
      value.forEach(zipCode => {
        // adding color and ID for filter
        colorCode.push(zipCode.zip, colors[index]);
        // Creating new layer data for each color based on internal zip-layer mapping
        if (!zipColors[colors[index]]) {
          zipColors[colors[index]] = [];
        }
        zipColors[colors[index]].push(zipCode.zip);
        zipCodes.push(zipCode.zip);
        zipCodeIds.push(zipCode.zip);
      });
    });
   ...
   /*if (flag !== 'zip') {
      ...
      map.setFilter('zipCode', zipCodes);
    }*/
