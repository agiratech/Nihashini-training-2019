...
export class AuditJobsFilterComponent implements OnInit, OnDestroy {
  ...
  loadAuditPlaces() {
    ...
    this.placesFilterService.getAuditPlaces(true, 0).pipe(takeUntil(this.unSubscribe)).subscribe(data => {
  ...
  public openPlaceDetails(node: AuditPlaceNode): void {
    if (node.level === 2) {
  ...
  onScroll(event) {
    // AS a quickFix added Jquery, We will remove later
    if (event.target) {
  ...
  loadMorePanels(node, fromNextPlace = false) {
    ...
    node.isLoading = true;
    this.placesFilterService.getAuditPlaces(true, 100, node.parent, node.id, paginationInfo.page)
      .pipe(
