...
export class PlacesFiltersComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    /**
     * To subscribe the applied place in place set dialog
     */
    this.placeFilterService
      .getSelectedPlaceSet()
      .pipe(takeWhile(() => this.unSubscribe))
      .subscribe(place => {
        if (place) {
          this.placeFilterService.setFilterSidenav({ open: true, tab: 'myPlaces' });
          this.selectPlaceID = place._id;
          this.onFilterByPlaceSet(place._id);
        }
      });
  ...
  private loadSession() {
    ...
    if (placesSession) {
      ...
      if (this.selectedTab === 1 && placesSession['placeSetsFilter'] && placesSession['placeSetsFilter']['place']) {
        ...
        this.selectPlaceID = this.filterData['place'];
  ...
  getPlacesDataFromAPI(filterData: Partial<PlaceFilterParams>, placeTypesInit = true, industriesInit = true) {
    ...
    /* this.placeFilterService.getPlaces(filterData)
      .subscribe(response => {
      // search key summary id to get bulk of searched records
      const filters = this.placeFilterService.getPlacesSession();
      filters['filters']['summaryId'] = response['summaryId'];
      this.placeFilterService.savePlacesSession('filters', filters['filters']);
      this.summary = response['summary'];
      this.placesResults = response;
      this.filterLevel = 2;
      // The below code is commented because of elastic search functionality.
      // It will be removed when integrating elastic search functionality completed
      // if (placeTypesInit) {
      //   this.placeTypes = response['filters']['place_types'];
      // }
      // this.filterOptions = response['filters'];
      // const states = response['filters']['states'];
      // const ids = response['filters']['ids'];
      // const geohash5 = response['filters']['geohash5'];
      // const geohash6 = response['filters']['geohash6'];
      // this.pushNationalData.emit({states: states, ids: ids, geohash5: geohash5, geohash6: geohash6});
      this.selectPlaceID = '';
      this.placeSetLevel = 0;
      this.placeFilterService.setFilterLevel({filterLevel: this.filterLevel});
    },
  ...
  private getNationalLevelData(query) {
    const totalQuery = JSON.parse(JSON.stringify(this.elasticsearch.addTotalQuery(query)));
    this.elasticsearch
      ...
      .subscribe(res => {
        if (res && res['hits'] && res['hits']['total'] && res['hits']['total']['value'] < 10000) {
          const state = this.elasticsearch.getDataFromElasticSearch(
            JSON.parse(JSON.stringify(this.elasticsearch.getNationalLevelData(query, 'state')))
          );
          const geohash5 = this.elasticsearch.getDataFromElasticSearch(
            JSON.parse(JSON.stringify(this.elasticsearch.getNationalLevelData(query, 'geohash5')))
          );
          const geohash6 = this.elasticsearch.getDataFromElasticSearch(
            JSON.parse(JSON.stringify(this.elasticsearch.getNationalLevelData(query, 'geohash6')))
          );
          const sfids = this.elasticsearch.getDataFromElasticSearch(
            JSON.parse(JSON.stringify(this.elasticsearch.getNationalLevelData(query, 'safegraphIds')))
          );
  ...
  public updatePolygonInfo(data) {
    this.updatedPlaceInfo = JSON.parse(JSON.stringify(data));
  }
