...
export class PlacesAssignjobComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('fileInp', { static: false }) selectedFiles: ElementRef;
  @ViewChild('uploadFormRef', { static: false }) uploadFormRef: NgForm;
  client;
  isAuditRole = false;
  ...
  public mapFields(formGroup: FormGroup): void {
    if (formGroup.valid) {
      ...
      matchedColumns['mappings'] = this.mappingsForm.getRawValue()['mappings'].filter((data) => data.source_key !== 'No Match');
      const mappedUnMapped = this.mappingsForm.getRawValue()['unmappings'].filter((data) => data.source_key !== 'No Match');
      matchedColumns['mappings'] = matchedColumns['mappings'].concat(mappedUnMapped);
      if (this.isAuditRequested && this.isAuditRole) {
        matchedColumns['audit_status'] = 'Requested';
      }
      this.placesFilterService.updateCsvFieldsMapping(
        this.selectedCustomerId,
        this.uploadedFileName,
        matchedColumns).pipe(takeUntil(this.unSubscribe)).subscribe(response => {
          this.selectedIndex = 2;
          this.cdRef.markForCheck();
        }, error => {
          if (error.error && error.error.code &&  error.error.code === 120032) {
