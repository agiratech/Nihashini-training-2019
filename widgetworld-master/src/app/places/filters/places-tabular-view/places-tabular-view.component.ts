...
export class PlacesTabularViewComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges {
  ...
  @ViewChild(MatSort, {static: false}) placeSort: MatSort;
  ...
  ngOnInit() {
    ...
    this.currentSortables = JSON.parse(JSON.stringify(this.sortables));
    const obj = {
      'field_name': '',
      'key': 'checked'
    };
  ...
  public customizeColumn() {
    let currentSortables = JSON.parse(JSON.stringify(this.currentSortables));
    const sortables = JSON.parse(JSON.stringify(this.sortables));
    sortables.map(sort => {
      sort.displayname = sort.field_name;
    });
